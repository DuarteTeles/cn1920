...
def buildDictionaries(texts):
    ...
    all_author_topic_pairs = [(a, t) for t in all_topics for a in all_authors]
    ...
    print("Total topic x author pairs:", len(all_author_topic_pairs))
        ...
        for (index, author) in enumerate(all_authors):
            ...
            for topic in all_topics:
                if author in time_author_topic[time] and topic in time_author_topic[time][author]:
                    ...
                    jat_tensor.write('\t' + str((time_author_topic[time][author][topic])))
                else:
                    jat_tensor.write("\t0")
            jat_tensor.write('\n')
